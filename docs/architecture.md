...
`AI_PROVIDER=api` retains the HTTP provider path. `AI_API_FORMAT=openai` selects Chat Completions-compatible payloads, while `AI_API_FORMAT=anthropic` selects Anthropic Messages payloads. The neutral `AI_API_URL`, `AI_API_MODEL`, and `AI_API_KEY` settings work for either format; the former `OPENAI_*` names remain read-only compatibility fallbacks and are removed on the next configuration save. Known global `AI_EFFORT` values are `low`, `medium`, `high`, `xhigh`, and `max`, but other non-empty strings pass through unchanged for model-specific or future levels. API mode defaults to `max` and sends the value as `reasoning_effort` for OpenAI-format requests or `output_config.effort` for Anthropic requests. Anthropic-compatible image requests allow up to 8192 response tokens while the system prompt asks the model to keep its final JSON within approximately 4096 tokens, leaving headroom for reasoning-heavy models; a true `max_tokens` stop is reported explicitly rather than as a generic JSON parse failure. It does not issue startup or periodic probe requests and does not switch credentials after a failure.
